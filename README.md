...
#### Arduino
I used the Arduino Micro for this project that has one hardware UART in addition to the one for the USB cable. This meant that I needed to put one of the Bluetooth adaptors on a SoftwareSerial connection. Let me save you a lot of time if you are going to use a different Arduino by sharing this link:
...
Other than that, I would say that the Arduino part of the solution was one of the simplest parts. The Arduino ecosystem is very well supported and coding is very immediate. In fact I decided to use Arduino rather than RPi for this element of the solution to avoid all the issues with Python setup on a RPi and the fact that the Bluetooth libraries don't seem that well covered on forums. The benefit of the HC-05 and HM-10 modules on Arduino is that they use a simple UART serial implementation which (for me at any rate) simplified things enormously.
